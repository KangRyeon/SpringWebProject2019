...
<html>
	...
	<body>
		...
		<%
			...
			int list_num = assignments.size();
			
			String dept_name[] = new String[list_num];
			String assg_name[] = new String[list_num];
			Date assg_start[] = new Date[list_num];
			Date assg_end[] = new Date[list_num];
			String assg_content[] = new String[list_num];
			
			
			for(int i=0; i<assignments.size(); i++) {
				dept_name[i] = assignments.get(i).getDept_name();
				assg_name[i] = assignments.get(i).getAssg_name();
				assg_start[i] = assignments.get(i).getAssg_start();
				assg_end[i] = assignments.get(i).getAssg_end();
				assg_content[i] = assignments.get(i).getAssg_content();
			}
		...
		<div class="table-wrapper">
			<div class="table-title">
				<div class="row">
					...
					<div class="col-md-8" style="padding:0;">
						<form action="/assignment" method="post" class="btn-group" style="width:600px;">
					...
					<%
						...
						for (int i = 0; i < dept_name.length; i++) {  // 0 ~ 끝까지 검색 -> 현재페이지*5 = 5, i*5 = 0,5,10
							if(depts == null || depts[0].equals("all") || dept_name[i].equals(depts[0])) { // depts[0] = 내가누른 all or 데이터베이스, 웹프로그래밍, 네트워크 버튼 중 하나	
								...
								out.println("<td><a href='assignment?select="+assg_name[i]+"'>" + assg_name[i] + "</a></td>");
								SimpleDateFormat f = new SimpleDateFormat("YYYY-MM-dd");
								out.println("<td>" + f.format(assg_start[i]) + "</td>");
								out.println("<td>" + f.format(assg_end[i]) + "</td>");
			...
			<div class="clearfix row">
				...
				<form action="/assignmentForm" method="post">
					<input type="submit" class="btn btn-default" value="과제물추가" style="border:0;">
				</form>
				</div>
				<form method="post" >
					<ul class="pagination">
		...
		<div class="table-wrapper">
			<table class="table table-striped">
				...
				<tbody>
					<%
						for (int i = 0; i < assg_name.length; i++) {  // 0 ~ 끝까지 검색 -> 현재페이지*5 = 5, i*5 = 0,5,10
							if(assg_name[i].equals(select)) { // 제목에 따라서 디테일 보여줌.	
								out.println("<tr>");
								out.println("<td>" + i + "</td>");
								out.println("<td>" + dept_name[i] + "</td>");
								out.println("<td>"+ assg_name[i] + "</td>");
								SimpleDateFormat f = new SimpleDateFormat("YYYY-MM-dd");
								out.println("<td>" + f.format(assg_start[i]) + "</td>");
								out.println("<td>" + f.format(assg_end[i]) + "</td>");
								//out.println("<td>" + assg_content[i] + "</td>");
								...
								out.println("<td colspan='5'>" + assg_content[i] + "</td>");
								out.println("</tr>");
							}
						}
					%>
